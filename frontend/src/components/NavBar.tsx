...
const NavBar: React.FC<{ hideAuthenticatedLinks?: boolean }> = ({ hideAuthenticatedLinks }) => {
  ...
  const renderNavItems = () =>
    navItems.map((item, idx) => {
      if (!user || hideAuthenticatedLinks) return null; // only hide links, not logo
  ...
  return (
    <nav className="bg-white shadow-md px-4 py-2 flex justify-between items-center">
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="absolute top-12 right-4 bg-white shadow-md rounded-md flex flex-col items-center p-4 gap-2 md:hidden">
          {renderNavItems()}
          {user && !hideAuthenticatedLinks && (
            <button
              onClick={handleLogout}
