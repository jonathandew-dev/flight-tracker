// src/modules/savedTrip/savedTrip.controller.ts
...
// POST /savedTrips
export const createSavedTrip = catchAsync(async (req: Request, res: Response) => {
  const { title, flights } = req.body;

  // Construct data using SavedTripUncheckedCreateInput
...
// GET /savedTrips
export const getAllSavedTrips = catchAsync(async (req: Request, res: Response) => {
  const trips = await savedTripService.getAllSavedTrips(req.userId);
  res.status(200).json({ trips });
});

// GET /savedTrips/:id
export const getSavedTripById = catchAsync(async (req: Request, res: Response) => {
  const trip = await savedTripService.getSavedTripById(req.params.id);
  res.status(200).json({ trip });
});

// PUT /savedTrips/:id
export const updateSavedTrip = catchAsync(async (req: Request, res: Response) => {
  const trip = await savedTripService.updateSavedTrip(req.params.id, req.body);
  res.status(200).json({ trip });
});

// DELETE /savedTrips/:id
export const deleteSavedTrip = catchAsync(async (req: Request, res: Response) => {
